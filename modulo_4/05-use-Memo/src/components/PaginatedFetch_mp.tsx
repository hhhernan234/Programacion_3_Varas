...
export default function PaginatedFetch_mp() {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', maxWidth: 540, margin: '0 auto', padding: 24 }}>
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Empleados de la Nómina</h2>
          ...
          {empleados.map(e => (
            <div key={e.id} style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: '#888' }}>@{e.username}</div>
              </div>
              <div style={{ fontSize: 13, color: '#555' }}>{e.email}</div>
            </div>
          ))}
        </div>
      )}
