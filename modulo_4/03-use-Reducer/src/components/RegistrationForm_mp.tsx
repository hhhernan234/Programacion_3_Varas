...
type FormAction =
  | { type: 'SET_FIELD'; field: keyof Pick<FormState, 'name' | 'apellido' | 'email' | 'password'>; value: string }
...
export default function RegistrationForm_mp() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Simulación de llamada a la API de nómina
    ...
    >
      {state.status === 'success' && (
        <div style={{ padding: 12, background: '#dcfce7', borderRadius: 6, color: '#166534' }}>
          ✅ Empleado registrado en la nómina
        </div>
      )}
